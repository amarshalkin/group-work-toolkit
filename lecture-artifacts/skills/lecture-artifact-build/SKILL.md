...
5. **Generate DATA JSON.** Read the schema thoroughly. Use the example as a shape reference. Extract content from the transcript that satisfies the schema. Pin general fields (`lecturer`, `lectureTitle`, `date`) from `program[i]` — never invent. For arrays with strict cardinality (e.g. "exactly 12 questions"), match exactly; if the transcript doesn't cover all entries, fill remaining ones with conservative typical content and add a note in the user-facing summary.
   ...
   - **Для `step-builder`** — в DATA добавь поле `voicesMeta`: `{s: {name, role}, c: {...}, a: {...}}` с именами и ролями трёх фактических голосов-комментаторов из лекции. Это поле обязательное. Также допустимо `DATA.labels` для переопределения UI-надписей под жанр (если не «урок»). См. `templates/step-builder/schema.md`.
...
8. **Build the event.json.** Compute marker values from `event` and `program[i]`:
   ...
   - `closing-title` (только для `manifesto`) ← лозунг-выжимка по теме события (см. schema.md «Закрывающий блок»). Inline `<b>`, `<br>` допустимы.
   - `closing-text` (только для `manifesto`) ← параграф-комментарий 2-3 предложения, плоский текст.
   - `links-eye-text` (только для `manifesto`) ← эпиграф «<N прописью> инструментов · ...», N = `data.links.length`. Число прописью.
   - `links-h-text` (только для `manifesto`) ← заголовок секции ссылок («Возьмите с собой <b>в ...</b>.»).
