...
## Универсальность кейсов и подписи

Шаблон работает не только со странами — это сравнительные кейсы любого жанра: подходы, методы, школы, регионы, эпохи. Поле `countries[]` — историческое имя; семантически это «`cases`». Подписи кнопок и счётчик в шапке колоды настраиваются под конкретную лекцию через 3 маркера + DATA.labels.

### Markers (evt:)

- `cases-deck-hint` ← подсказка-эпиграф над колодой. Шаблон: «<N прописью> <случаев в род. падеже мн. ч.> · свайпайте горизонтально». Например: «8 стран · свайпайте горизонтально», «6 подходов · свайпайте горизонтально». N = `countries.length`.
- `cases-back-label` ← подпись кнопки возврата к колоде. Дательный падеж + предмет в род. падеже. Например: «к картотеке», «к подходам», «к школам».

### DATA.labels (для JS-рендера)

- `caseDetailLabel` ← подпись кнопки «открыть карточку» после квиза. Дательный падеж ед. ч. согласованный с предметом. Например: «к стране →», «к подходу →», «к школе →».

**Дисциплина чисел:** `cases-deck-hint` ОБЯЗАН содержать число прописью, совпадающее с `countries.length`. `validate-counts.mjs` это контролирует.
...
```json
{
  ...
  "properties": {
    ...
    "labels": {
      ...
      "additionalProperties": false,
      "properties": {
        "caseDetailLabel": { "type": "string", "minLength": 2 }
      }
    }
  }
}
