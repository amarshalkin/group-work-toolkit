...
Главный `<h1>` страницы (внутри hero) подменяется маркером `evt:page-title`. Шаблон стиля: «Где вы на карте/спектре <темы>». Может содержать `<br>`, `<span class="accent">`, `<em>`. Тон вопросительный, обращён к читателю.

Пример: `Где вы<br>на территории <span class="accent">цифровой&nbsp;этики</span>`

**Дисциплина чисел:** не упоминай в заголовке количества вопросов/уровней/территорий — эти числа уже выводятся в `<span class="num">`-блоке шаблона из реальных длин массивов. Если всё-таки упоминаешь — должно совпадать (12 / 5 / 8).

## Подзаголовок-эпиграф (lecture-title)

Маленький эпиграф над `<h1>` (внутри `.intro-eyebrow`, обёрнутый `<b>…</b>`) подменяется маркером `evt:lecture-title`. Стиль: полное название лекции с годом. `<b>` снаружи маркера, текст внутри.

Пример: `Карта педагогики XXI века 2026`

## Event-маркеры, которые подменяет движок

- `event-name` ← `event.name`
- `event-year` ← год из `event.month_year`
- `act-title` ← короткое имя такта для topbar (например, «Самотест»)
- `lecture-title` ← полное название лекции с годом (см. выше)
- `page-title` ← заголовок страницы (см. выше)
