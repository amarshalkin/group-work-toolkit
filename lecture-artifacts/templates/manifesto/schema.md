...
## Закрывающий блок и ссылки

Шаблон содержит закрывающий лозунг + параграф размышления + секцию ссылок на остальные артефакты события. Все 4 строки настраиваются через markers:

### `evt:closing-title`

Лозунг-выжимка манифеста, формат «<отрицание> — <утверждение>». 1-2 строки, может содержать `<b>`, `<br>`. Тон декларативный.

Примеры:
- «Не нужен <b>идеальный</b> учитель.<br>Нужен — <b>проснувшийся</b>.» (исходное)
- «Не нужен <b>правильный</b> урок.<br>Нужен — <b>живой</b>.»

### `evt:closing-text`

Параграф-комментарий под лозунгом. 2-3 предложения, плоский текст, без HTML.

Пример: «Если из всех тезисов вы возьмёте хотя бы один и сделаете один шаг на этой неделе — событие состоялось.»

### `evt:links-eye-text`

Эпиграф-счётчик над сеткой ссылок. Шаблон: «<N прописью> инструментов · к каждой лекции» либо иначе. N = `DATA.links.length`.

**Дисциплина чисел**: число должно совпадать с фактической длиной массива ссылок.

### `evt:links-h-text`

Заголовок секции ссылок. Тон побудительный, 1 строка, может содержать `<b>`.

Примеры:
- «Возьмите с собой <b>в понедельник</b>.» (исходное)
- «Заберите <b>в свой класс</b>.»
